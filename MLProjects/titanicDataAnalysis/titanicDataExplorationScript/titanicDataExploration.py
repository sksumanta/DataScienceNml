...
confusionMatrix = confusion_matrix(ytest , baseLine.predict(Xtest) )
print(confusionMatrix)   # from the confusion matrix we can see true negetive
            #  is more means most of the people are not servived. So if we
            # reduce the  false negetive then we will get the better model.

# Select the MachineLearning Algorithm
'''
'''
If the output/dependent variable has Discrete labels then we need to 
follow Classification algorithm.
    Classification algorithim :- KNN (k Nearest Neighbors) , Logistic regression
            , Naive Bayes , decision tree , Random forest , Support vector machine

In Classification model, we essentially try to find a decision boundary 
that can separate each classes ( in our example -- servived and not servived). 

If the output/dependent variable have continuous values then we need to 
follow Regression algorithm.
    Regression algorithm :- Linear regression , Support vector machine 

If there is unsupervised data and from the pattern we need to segregate 
them then we can use Clustering algorithm.

'''
'''
# Lets consider the Logistic regression Algorithm to perform prediction.
    
from sklearn.linear_model import LogisticRegression
logisticRegr = LogisticRegression()
logisticRegr.fit(Xtrain, ytrain)
yLregPredict = logisticRegr.predict(Xtest)  # The yPredict will be the output for the future data
     

# Accuracy of the model 

from sklearn.metrics import accuracy_score
accuracy_score(ytest, yLregPredict)

# confusion metrics

from sklearn.metrics import confusion_matrix, accuracy_score,classification_report

confusionMatrixLreg = confusion_matrix(ytest , yLregPredict )
print(confusionMatrixLreg)

# classification report
print(classification_report(ytest , yLregPredict)) 


# Lets handle the underfitting and overfitting for the model 
# and measure the accuracy if it increaseing or not.

# For this we can use hyper-parameter optimization that is Regularization
# There is different regularization technique 
            # gridSearch , randomSerach , Gaussian optimization 
# we can also use k-fold cross validdation.

# L2 Regularization with GridSearch technique 
'''
'''
On above logistict regression  we have logisticRegr = LogisticRegression()

To perform L2 Regularization using gridSearch we will pass below
parameters in logistic regression.
The parameters are   alpha/C  ,  random_state , penalty 
'''
'''
...
# Regularization with randomSerach technique 
from sklearn.linear_model import LogisticRegression
logisReg = LogisticRegression(random_state=0)
...
randomSarch = RandomizedSearchCV(logisReg , param_distributions=gridParamters ,n_iter=4)
randomSarch.fit(Xtrain, ytrain)

randomSarch.best_params_

yPredict = randomSarch.predict(Xtest)
